...
export class TranslationService {
     ...
     */
    async submitIdmlForTranslation(idmlBuffer, sourceLanguage, targetLanguage, filename) {
        // Validate languages
        const warnings = [];
     ...
     * @param targetLanguage - The target language code
     */
    async getCompletedTranslation(originalIdmlBuffer, googleDocId, targetLanguage) {
     ...
     */
    async translateIdmlFile(idmlBuffer, sourceLanguage, targetLanguage, filename, googleDocId) {
        ...
        const translationResponse = await this.webhookService.waitForTranslationCompletion(googleDocId, targetLanguage);
    ...
    mergeTranslations(originalTextBoxes, translationResponse) {
    ...
    async extractTextOnly(idmlBuffer) {
        await this.idmlParser.loadIdmlFile(idmlBuffer);
        const document = await this.idmlParser.parseDocument();
        return document.textBoxes;
    }
    async updateIdmlWithTranslations(originalIdmlBuffer, translations) {
        await this.idmlParser.loadIdmlFile(originalIdmlBuffer);
        const document = await this.idmlParser.parseDocument();
